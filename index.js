...
async function runZulip(zulipApi, web) {
    ...
    while (1) { //loop on events.retrieve
        ...
        events.events.forEach(async e => {
            ...
            if (e.message.sender_email.includes('bot@zulip')) // === "slack-bridge-bot@zulip.superbiche.co")
                return;
...
async function forwardToZulip(zulipApi, slackMessage) {
    ...
    if (b) {
        ...
        var zstream = "testing";
        var zsubject = "pipedream";
        var zcreds= "/home/maneesh/programming/zulip-slack-sync/creds/ManeeshSlackBot.zuliprc"
        var zbotemail = "ManeeshSlackHook-bot@zulip.pavlok.chat";
        var zbotapikey = "jLjYxfyFyiT9KZ6dSrZeUpXEPOtijryG";
        var zmessage = slackMessage.text;
        var zulipsendstr = "zstring: " + `zulip-send --config-file ${zcreds} site zulip.pavlok.chat --subject ${zsubject} -m ${zmessage}`;
        
        //        zulip-send --stream testing --subject pipedream --user ManeeshSlackHook-bot@zulip.pavlok.chat      --api-key jLjYxfyFyiT9KZ6dSrZeUpXEPOtijryG -m  'hi i am from commandline' --site zulip.pavlok.chat;


        console.log(zulipsendstr);

        console.log("slackMessage: " + util.inspect(slackMessage, {showHidden: false, depth: null, colors: true}));


        exec(`zulip-send --config-file ${zcreds} --site zulip.pavlok.chat --stream ${zstream} --subject ${zsubject} -m "${zmessage}"`, (error, stdout, stderr) => {
            if (error) {
    ...
    //  zulip-send --stream zstream --subject zsubject    --user zbotemail --api-key zbotapikey —message zmessage jLjYxfyFyiT9KZ6dSrZeUpXEPOtijryG
...
async function forwardToSlack(web, zulipMessage) {
    ...
    console.log("zulipmessaee: " + util.inspect(zulipMessage, {showHidden: false, depth: null, colors: true}));
    if (b) {
        web.chat.postMessage({ channel: b.slack_channel_id, text: zulipMessage.content, username: zulipMessage.sender_email });
